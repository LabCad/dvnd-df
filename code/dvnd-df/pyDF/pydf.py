...
class Scheduler:
	...
	def propagate_op(self, oper):
		dst = self.graph.nodes[oper.dstid]
		if self.mpi_rank != None:
			if dst.affinity != None and (dst.affinity[0]/self.n_workers) == self.mpi_rank or dst.affinity == None and self.mpi_rank == 0:
	...
	def terminate_workers(self, workers):
		print "Terminating workers %s %d %d (rank = %d)" %(self.all_idle(), self.operq.qsize(), len(self.tasks), self.mpi_rank)
		#if self.mpi_rank == 0:
	...
	def main_loop(self):
		...
		while len(tasks) > 0 or not self.all_idle() or operq.qsize() > 0 or (self.mpi_rank > 0 and self.keep_working):
			...
			if wid not in self.waiting and opersmsg[0].request_task:
				...
				if self.mpi_rank != None and self.mpi_rank > 0 and len(tasks) == 0:
			...
			while len(tasks) > 0 and len(self.waiting) > 0:
				...
				if self.mpi_rank == None or  (wid/self.n_workers == self.mpi_rank): #local worker
					if self.mpi_rank != None:
