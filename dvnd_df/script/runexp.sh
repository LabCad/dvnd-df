...
if [ $# -gt 3 ]; then
	...
	home_src=$home_p"code/dvnd-df/dvnd-df/"
	home_doc=$home_p"doc/"
	...
	do
		...
		do
			...
			then
				...
				time mpirun -np $num_proc --hostfile $home_script""$host_file python $home_src"main.py" -sii $i -mpi -sog -n $num_workes -in $filei -mg -dc 2 -s ${solver_name:0:4} -p ml > $home_doc"results/"$file_name".out" 2> $home_doc"results/"$file_name".log"
