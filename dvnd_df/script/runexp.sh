...
if [ $# -gt 3 ]; then
	...
	file_list=()
	problem=
	home_p="/home/rodolfo/parco/dvnd-df/"
	...
	do
		...
		do
			...
			then
				...
				time mpirun.mpich -np $num_proc --hostfile $home_script""$host_file python $home_src"main.py" -sii $i -mpi -sog -n $num_workes -in $filei -mg -dc 2 -s ${solver_name:0:4} -p ml > $home_doc"results/"$file_name".out" 2> $home_doc"results/"$file_name".log"
			...
			then
				...
				#time mpirun.mpich -np $num_proc --hostfile $home_script""$host_file python $home_src"/main.py" -sii $i -mpi -n $num_workes -in $filei -mg -dc 2 -s $solver_name -p ml > $file_out".out" 2> $file_out".log"
				time mpirun.mpich -np $num_proc --hostfile $home_script""$host_file python $home_src"/main.py" -sii -2 -mpi -n $num_workes -in $filei -dc 2 -s $solver_name -p ml > $file_out".out" 2> $file_out".log"
			fi
		done
	done
