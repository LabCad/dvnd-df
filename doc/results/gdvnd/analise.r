...
dvndGdvndSameInitial = data.table(read.csv(file="dvndGdvnd-sameInitial.csv", head=TRUE, sep=";"))
dvndGdvndRandInitial = data.table(read.csv(file="dvndGdvnd-randInitial.csv", head=TRUE, sep=";"))
rvndSameInitial = data.table(read.csv(file="rvnd-sameInitial.csv", head=TRUE, sep=";"))
...
dvndGdvndData = merge(dvndGdvndSameInitial, dvndGdvndRandInitial, rvndSameInitial, all=TRUE)
  ...
  for (iniMethod in c("same", "rand")) {
    ...
    #foreach(draw_inum=0:7, .combine=cbind) %dopar% {
      ...
      for (draw_w in 4:5) {
        ...
        # 
        #   ggsave(paste("chart/", iniMethod, "_time_in", draw_inum, "n", draw_n, "w", draw_w, sep=""), plot = mychart, device="png")
        # }
        # 
        ...
        # 
        #   ggsave(paste("chart/", iniMethod, "_imp_in", draw_inum, "n", draw_n, "w", draw_w, sep=""), plot = mychart, device="png")
        # }
        ...
        if (length(data_src$sample) > 0) {
          ...
          ggsave(paste("chart/", iniMethod, "_count_in", draw_inum, "n", draw_n, "w", draw_w, sep=""), plot = mychart, device="png")
        }
      }
    }
  }
